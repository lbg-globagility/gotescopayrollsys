...
Public Class PayStubForm
    ...
    Private Sub SomeDgvWorks()
        ...
        If x = 0 Then
            ...
            End If
            employeepicture = New SQLQueryToDatatable("SELECT RowID,Image FROM employee WHERE Image IS NOT NULL AND OrganizationID=" & org_rowid & ";").ResultTable
    ...
    Private Sub dgvpayper_SelectionChanged(sender As Object, e As EventArgs) 'Handles dgvpayper.SelectionChanged
        ...
        If dgvpayper.RowCount <> 0 Then
            With dgvpayper.CurrentRow
                ...
                paypFrom = MYSQLDateFormat(CDate(.Cells("Column2").Value))

                paypTo = MYSQLDateFormat(CDate(.Cells("Column3").Value))
    ...
    Function COUNT_employeeabsent(Optional EmpID As Object = Nothing,
        ...
        Try
            ...
            With cmd
                ...
                returnval = If(datread.Read = True, If(IsDBNull(datread.GetString(0)), "0.00", datread.GetString(0).ToString), "0.00")
    ...
    Function GET_employeerateperhour(Optional EmpID As Object = Nothing,
        ...
        Try
            ...
            With cmd
                ...
                rate_perhour = If(datread.Read = True, If(IsDBNull(datread.GetString(0)), "0", datread.GetString(0).ToString), "0")
...
#Region "LEAVE_gainingbalance"
    ...
    Private Async Function GainingLeaveBalancesAsync() As Task
                ...
                If dgvemployees.RowCount <> 0 Then
                    ...
                    For Each dgvrow As DataGridViewRow In dgvLoanList.Rows
                        If dgvrow.Index = 0 Then
                            ...
                            objText.Text = dgvrow.Cells("c_loantype").Value & " loan " & tabchar & dgvrow.Cells("c_totballeft").Value
                        ...
                        Else
                            objText.Text &= vbNewLine & dgvrow.Cells("c_loantype").Value & " loan " & tabchar & dgvrow.Cells("c_totballeft").Value
                    ...
                    For Each dgvrow As DataGridViewRow In dgvempbon.Rows
                        If dgvrow.Index = 0 Then
                            objText.Text = dgvrow.Cells("bon_Type").Value

                            bonvalues.Text = "₱ " & FormatNumber(Val(dgvrow.Cells("bon_Amount").Value), 2)
                        Else
                            objText.Text &= vbNewLine & dgvrow.Cells("bon_Type").Value
        ...
        For Each drow As DataRow In employee_dattab.Rows
                    ...
                    If drow("EmployeeType").ToString = "Fixed" Then
                        ...
                        If ValNoComma(.Cells("etent_TotalDayPay").Value) = 0 Then
                            If drow("PayFrequencyID").ToString = 1 Then
                                ...
                                newdatrow("Column4") = "₱ " & FormatNumber(theEmpBasicPayFix, 2)

                                newdatrow("Column15") = "₱ " & FormatNumber(theEmpBasicPayFix, 2)

                                newdatrow("Column16") = "₱ " & FormatNumber(theEmpBasicPayFix, 2)
                            Else
                                ...
                                newdatrow("Column4") = "₱ " & FormatNumber(totbasicpay, 2)

                                newdatrow("Column16") = "₱ " & FormatNumber(totbasicpay, 2)
                            End If
                        Else
                        ...
                        End If
                    Else
                        newdatrow("Column15") = "₱ " & FormatNumber(ValNoComma(.Cells("etent_TotalDayPay").Value), 2)

                        newdatrow("Column16") = "₱ " & FormatNumber(ValNoComma(.Cells("etent_TotalDayPay").Value), 2)
            ...
            For Each loanrow As DataRow In sel_employeeloanfulldetails
                ...
                Else
                    ...
                    newdatrow("Column35") &= vbNewLine & loanrow("PartNo").ToString & " loan"
            ...
            For Each dgvrow As DataGridViewRow In dgvempbon.Rows 'Bonuses
                ...
                Else
                    newdatrow("Column36") &= vbNewLine & dgvrow.Cells("bon_Type").Value
            ...
            For Each drow As DataRow In employee_dattab.Rows
                ...
                If drow("EmployeeType").ToString = "Fixed" And drow("EmploymentStatus").ToString = "Regular" Then
                    ...
                    For Each drowsal In rowempsal
                        ...
                        Dim taxab_salval = Trim(emptaxabsal)
                ...
                For Each drowtotdaypay In emptotdaypay
                    ...
                    For Each drowsal In rowempsal
                        ...
                        And drow("PayFrequencyID").ToString = "4" Then
                            ...
                            emptaxabsal = grossincome - (pstub_TotalEmpPhilhealth)
                        ...
                        If drow("EmploymentStatus").ToString = "Regular" Then 'accruance of leave are for regular employees only
                            ...
                            If sel_empleave.Count = 0 Then
                                ...
                                EXECQUER("UPDATE employee SET" &
                                         ...
                                         " WHERE RowID='" & drow("RowID").ToString & "';")
        ...
        If Not hasError Then
            Try
                ...
                For Each dgvRow As DataGridViewRow In dgvAdjustments.Rows
                    ...
                    If productRowID IsNot Nothing And dgvRow.IsNewRow = False Then
            ...
            For Each drow As DataRow In employee_dattab.Rows
                ...
                For Each drowtotdaypay In emptotdaypay
                    ...
                    For Each drowsal In rowempsal
                        ...
                        Dim amount_used_to_get_sss_contrib = (monthly_computed_salary + addtl_taxable_daily_allowance)
                        ...
                        Dim phh_ee = ValNoComma(phh.Compute("MIN(EmployeeShare)", "SalaryRangeFrom <= " & amount_used_to_get_sss_contrib & " AND " & amount_used_to_get_sss_contrib & " <= SalaryRangeTo"))
                        Dim phh_er = ValNoComma(phh.Compute("MIN(EmployerShare)", "SalaryRangeFrom <= " & amount_used_to_get_sss_contrib & " AND " & amount_used_to_get_sss_contrib & " <= SalaryRangeTo"))
                ...
                Dim last_tax_amount = tax_amount
