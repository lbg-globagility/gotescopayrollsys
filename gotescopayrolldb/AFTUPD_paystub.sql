...
#	SET @fsdfdsfSalaryId=0;
		SELECT (TrueSalary / PAYFREQUENCY_DIVISOR(pftype)), es.RowID FROM employeesalary es WHERE es.EmployeeID=NEW.EmployeeID AND es.OrganizationID=NEW.OrganizationID AND (es.EffectiveDateFrom >= NEW.PayFromDate OR IFNULL(es.EffectiveDateTo,NEW.PayToDate) >= NEW.PayFromDate) AND (es.EffectiveDateFrom <= NEW.PayToDate OR IFNULL(es.EffectiveDateTo,NEW.PayToDate) <= NEW.PayToDate) ORDER BY es.EffectiveDateFrom DESC LIMIT 1 INTO totalworkamount,@fsdfdsfSalaryId;
#	SET @fsdfdsfSalary=totalworkamount;
...
#		IF NEW.EmployeeID = 475 THEN SELECT @fsdfdsfSalary, @fsdfdsfSalaryId, 'less', 'add' INTO OUTFILE 'D:/test.txt'; END IF;
		IF totalworkamount IS NULL THEN
