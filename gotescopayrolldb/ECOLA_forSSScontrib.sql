...
-- Dumping structure for procedure gotescopayrolldb_latest.ECOLA_forSSScontrib
DROP PROCEDURE IF EXISTS `ECOLA_forSSScontrib`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `ECOLA_forSSScontrib`(IN `og_rowid` INT, IN `date_from` DATE, IN `date_to` DATE)
    DETERMINISTIC
BEGIN
...
END IF;
   ...
   , e.WorkDaysPerYear
   , 0 `PAYFREQDIV`
	, (ea.AllowanceAmount - IFNULL(ROUND((ea.AllowanceAmount / (e.WorkDaysPerYear / (month_count_peryear * divided_into_half))), 2) * IFNULL(xy.`AbsentCount`,0), 0)) `TotalAllowanceAmount`
