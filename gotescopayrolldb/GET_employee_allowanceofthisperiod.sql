...
DELIMITER //
CREATE PROCEDURE `GET_employee_allowanceofthisperiod`(IN `OrganizID` INT, IN `AllowanceFrequenzy` VARCHAR(50), IN `IsTaxable` CHAR(1), IN `DatePayFrom` DATE, IN `DatePayTo` DATE)
    DETERMINISTIC
BEGIN
...
UNION
	...
	,(i.TotalAllowanceAmt / default_min_hrswork) `HourlyAllowance`
	# ,i.AllowanceAmount - (SUM(i.HoursToLess) * (i.TotalAllowanceAmt / default_min_hrswork)) AS TotalAllowanceAmount
...
ELSE
	...
	INNER JOIN employee e ON e.RowID=i.EmployeeID AND FIND_IN_SET(e.EmploymentStatus, UNEMPLOYEMENT_STATUSES()) = 0
	INNER JOIN product p ON p.RowID=i.ProductID AND p.`Status`=IsTaxable
	...
	WHERE i.`Date` BETWEEN DatePayFrom AND DatePayTo
	AND i.TotalAllowanceAmt > 0
	AND i.OrganizationID=OrganizID
	;
