...
DELIMITER //
DROP PROCEDURE IF EXISTS `GET_employeeallowance_UseInSss`;
CREATE PROCEDURE `GET_employeeallowance_UseInSss`(
	IN `orgId` INT,
	IN `periodId` INT
)
BEGIN
...
DROP TEMPORARY TABLE IF EXISTS `customemployee`;
CREATE TEMPORARY TABLE IF NOT EXISTS `customemployee`
SELECT
...
/*DAILY*/
DROP TEMPORARY TABLE IF EXISTS `allowancedaily`;
CREATE TEMPORARY TABLE IF NOT EXISTS `allowancedaily`
SELECT
i.EmployeeID
,i.`Date`
,0 `Equatn`
...
INNER JOIN `customemployee` e ON e.RowID=i.EmployeeID AND FIND_IN_SET(e.EmploymentStatus, UNEMPLOYEMENT_STATUSES()) = 0
AND IF(e.`IsPerPayPeriod`, TRUE, IF(e.`IsEndOfMonth`=@isPeriodEndOfMonth, TRUE, IF(e.`IsFirstHalf`=@isPeriodFirstHalf, TRUE, FALSE)))

INNER JOIN product p ON p.RowID=i.ProductID AND p.UseInSss=@useInSss AND p.RowID NOT IN (@ecoalId)
...
/*SEMI-MONTHLY*/
DROP TEMPORARY TABLE IF EXISTS `allowancesemimonthly`;
CREATE TEMPORARY TABLE IF NOT EXISTS `allowancesemimonthly`
SELECT
i.*,
...
FROM (SELECT
		...
		INNER JOIN product p ON p.RowID=x.ProductID AND p.UseInSss=@useInSss AND p.RowID NOT IN (@ecoalId)

		INNER JOIN `customemployee` e ON e.RowID=x.EmployeeID AND FIND_IN_SET(e.EmploymentStatus, UNEMPLOYEMENT_STATUSES()) = 0
		AND IF(e.`IsPerPayPeriod`, TRUE, IF(e.`IsEndOfMonth`=@isPeriodEndOfMonth, TRUE, IF(e.`IsFirstHalf`=@isPeriodFirstHalf, TRUE, FALSE)))
...
DROP TEMPORARY TABLE IF EXISTS `allowancemonthly`;
CREATE TEMPORARY TABLE IF NOT EXISTS `allowancemonthly`
SELECT
...
INNER JOIN `customemployee` e ON e.RowID=et.EmployeeID AND FIND_IN_SET(e.EmploymentStatus, UNEMPLOYEMENT_STATUSES()) = 0
AND IF(e.`IsPerPayPeriod`, TRUE, IF(e.`IsEndOfMonth`=@isPeriodEndOfMonth, TRUE, IF(e.`IsFirstHalf`=@isPeriodFirstHalf, TRUE, FALSE)))
...
INNER JOIN product p ON p.RowID=ea.ProductID AND p.UseInSss=@useInSss AND p.RowID NOT IN (@ecoalId)
#WHERE et.OrganizationID=@orgId AND et.`Date` BETWEEN @firstdate AND @dateto
;

/*ONCE*/
DROP TEMPORARY TABLE IF EXISTS `allowanceonce`;
CREATE TEMPORARY TABLE IF NOT EXISTS `allowanceonce`
SELECT SUM(IFNULL(ea.AllowanceAmount,0)) `TotalAllowanceAmount`
,ea.EmployeeID
FROM employeeallowance ea

INNER JOIN `customemployee` e ON e.RowID=ea.EmployeeID AND FIND_IN_SET(e.EmploymentStatus, UNEMPLOYEMENT_STATUSES()) = 0
AND IF(e.`IsPerPayPeriod`, TRUE, IF(e.`IsEndOfMonth`=@isPeriodEndOfMonth, TRUE, IF(e.`IsFirstHalf`=@isPeriodFirstHalf, TRUE, FALSE)))
...
#DROP TEMPORARY TABLE IF EXISTS `totalallowanceuseinsss`;
#CREATE TEMPORARY TABLE IF NOT EXISTS `totalallowanceuseinsss`
SELECT
i.EmployeeID,
SUM(i.TotalAllowanceAmount) `TotalAllowanceAmount`
FROM (SELECT @dailyFrequency `Frequency`, EmployeeID, TotalAllowanceAmount FROM `allowancedaily`
		UNION
		SELECT @semiMonthlyFrequency `Frequency`, EmployeeID, TotalAllowanceAmount FROM `allowancesemimonthly`
		UNION
		SELECT @monthlyFrequency `Frequency`, EmployeeID, TotalAllowanceAmount FROM `allowancemonthly`
		UNION
		SELECT @onceFrequency `Frequency`, EmployeeID, TotalAllowanceAmount FROM `allowanceonce`
		) i
