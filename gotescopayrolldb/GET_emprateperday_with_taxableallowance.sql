...
-- Dumping structure for function gotescopayrolldb_server.GET_emprateperday_with_taxableallowance
DROP FUNCTION IF EXISTS `GET_emprateperday_with_taxableallowance`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `GET_emprateperday_with_taxableallowance`(`EmpID` INT, `OrgID` INT, `paramDate` DATE) RETURNS decimal(11,6)
    DETERMINISTIC
BEGIN
...
IF emptype IN ('Fixed','Monthly') THEN
		...
		ELSEIF emptype = 'Daily' THEN
			...
			INNER JOIN payperiod pp ON pp.OrganizationID=ea.OrganizationID AND pp.TotalGrossSalary=e.PayFrequencyID AND paramDate BETWEEN pp.PayFromDate AND pp.PayToDate
			WHERE ea.EmployeeID=EmpID
			AND ea.OrganizationID=OrgID
			AND ea.TaxableFlag='1'
			AND ea.AllowanceFrequency='Daily'
			AND (ea.EffectiveStartDate >= pp.PayFromDate OR ea.EffectiveEndDate >= pp.PayFromDate)
			AND (ea.EffectiveStartDate <= pp.PayToDate OR ea.EffectiveEndDate <= pp.PayToDate)
			INTO dailyrate;
			
			SET dailyrate = IFNULL(dailyrate,0) + empBasicPay;
