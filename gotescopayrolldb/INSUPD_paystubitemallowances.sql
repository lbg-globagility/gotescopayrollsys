...
DELIMITER //
CREATE PROCEDURE `INSUPD_paystubitemallowances`(
	IN `og_rowid` INT,
	IN `e_rowid` INT,
	IN `pp_rowid` INT,
	IN `user_rowid` INT
...
DROP TEMPORARY TABLE IF EXISTS semimonthlyallowanceforthisperiod; DROP TABLE IF EXISTS semimonthlyallowanceforthisperiod;
CREATE TEMPORARY TABLE semimonthlyallowanceforthisperiod
SELECT i.ProductID
,og_rowid `OrganizationID`
,CURRENT_TIMESTAMP() `Created`
,user_rowid `CreatedBy`
,ps_rowid `PayStubID`
,ROUND(i.`AllowanceAmount`, 2) `AllowanceAmount`
,FALSE `Undeclared`
...
#			,i.AllowanceAmount - TRIM(SUM( IFNULL((i.HoursToLess * (NULLIF(i.DailyAllowance, 0) / default_min_hrswork)), 0) ))+0 `AllowanceAmount`
...
#		AND NULLIF(i.DailyAllowance, 0) IS NOT NULL
		GROUP BY i.ProductID, i.TaxableFlag
#		GROUP BY i.EmployeeID, i.ProductID, i.TaxableFlag
		) i
#WHERE i.EmployeeID=e_rowid
;

# INSERT LOW_PRIORITY INTO paystubitem(`ProductID`,`OrganizationID`,`Created`,`CreatedBy`,`PayStubID`,`PayAmount`,`Undeclared`)
INSERT INTO paystubitem(`ProductID`,`OrganizationID`,`Created`,`CreatedBy`,`PayStubID`,`PayAmount`,`Undeclared`)
SELECT * FROM semimonthlyallowanceforthisperiod i
ON
DUPLICATE
KEY
UPDATE
