...
-- Dumping structure for procedure gotescopayrolldb_latest.LEAVE_gainingbalance
...
IF curr_year = YEAR(CURDATE()) THEN
	...
	SELECT MIN(ps.PayFromDate) FROM (SELECT * FROM paystub WHERE OrganizationID=OrganizID AND EmployeeID=EmpRowID) ps INNER JOIN payperiod pp ON pp.TotalGrossSalary = 1 AND pp.RowID=ps.PayPeriodID AND pp.`Year`=curr_year AND pp.OrganizationID=ps.OrganizationID WHERE ps.OrganizationID=OrganizID INTO minimum_date;
