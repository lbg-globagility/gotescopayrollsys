...
#			, ROUND(IFNULL(esa.BasicPay * IFNULL(esad.Percentage, 1), 0), 2) `Basic rate` # `DatCol21` # `BasicPay`
			, ROUND((IF(isActual, esa.TrueSalary, esa.Salary) / 2), 2) `Basic rate` # `DatCol21` # `BasicPay`
...
UNION
	...
	FROM (
			...
			# , IFNULL(esa.BasicPay, 0) `DatCol21` # `BasicPay`
			# , SUM(etp.BasicDayPay) `BasicPay`
#			, ROUND(IFNULL(esad.BasicPay * IFNULL(esad.Percentage, 1), 0), 2) `Basic rate` # `DatCol21` # `BasicPay`
#			, ROUND(IFNULL(esad.BasicPay * IFNULL(esad.Percentage, 1), 0), 2) `Regular`
			, ( @basicPay := ROUND((IF(isActual, esa.TrueSalary, esa.Salary) / 2), 2) ) `Basic rate`
			#IFNULL(@basicPay,1.11)
			, ( ROUND((IF(isActual, esa.TrueSalary, esa.Salary) / 2), 2)
			    - (SUM(IFNULL(et.HoursLateAmount,0))
				    + SUM(IFNULL(et.UndertimeHoursAmount,0))
					 + SUM(IFNULL(et.Absent,0))
