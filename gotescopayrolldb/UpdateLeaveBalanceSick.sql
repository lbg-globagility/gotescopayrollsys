...
DROP PROCEDURE IF EXISTS `UpdateLeaveBalanceSick`;
DELIMITER //
CREATE PROCEDURE `UpdateLeaveBalanceSick`(
	IN `OrganizID` INT,
	IN `UserId` INT,
	IN `PeriodId` INT,
	IN `startDate` DATE
...
#	, @i := (SELECT MIN(ProperLeaveBalance) FROM leavebalancepredict ii WHERE ii.PayperiodID=ps.PayPeriodID AND ii.EmployeeID=ps.EmployeeID AND ii.LeaveType=p.PartNo) `ProperLeaveBalance`
	...
	INNER JOIN product p ON p.RowID=psi.ProductID AND p.PartNo=@leaveType # Sick leave
