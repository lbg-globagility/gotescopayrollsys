...
DELIMITER //
CREATE PROCEDURE `VIEW_paystubitem_declared`(
	IN `OrganizID` INT,
	IN `EmpRowID` INT,
...
#				, SUM(etea.RegularHoursAmount) AS RegularHoursAmount
				, SUM(IF(etea.IsValidForHolidayPayment AND etea.IsDaily, IF(etea.IsLegalHoliday, etea.AddedHolidayPayAmount, etea.RegularHoursAmount), etea.RegularHoursAmount)) AS RegularHoursAmount
				...
				, SUM(IF(etea.IsValidForHolidayPayment,
							IF(etea.IsLegalHoliday OR (etea.IsSpecialHoliday AND etea.IsMonthly AND etea.RegularHoursWorked = 0),
								IF(etea.IsFixed AND etea.RegularHoursWorked = 0, 0, etea.DailyRate),
								0),
