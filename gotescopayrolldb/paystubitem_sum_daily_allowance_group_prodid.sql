...
-- Dumping structure for view gotescopayrolldb_server.paystubitem_sum_daily_allowance_group_prodid
				...
				, IF(pr.PayType='Special Non-Working Holiday' AND e.CalcSpecialHoliday = '1'
					...
					, IF(pr.PayType='Special Non-Working Holiday' AND e.CalcSpecialHoliday = '0'
						...
						, IF(pr.PayType='Regular Holiday' AND e.CalcHoliday = '1'
							, et.HolidayPayAmount + ((et.VacationLeaveHours + et.SickLeaveHours + et.MaternityLeaveHours + et.OtherLeaveHours) * (GET_employeerateperday(et.EmployeeID,et.OrganizationID,et.`Date`) / IFNULL(sh.DivisorToDailyRate,0)))
							, 0)))) / GET_employeerateperday(et.EmployeeID,et.OrganizationID,et.`Date`)) * ea.AllowanceAmount) AS TotalAllowanceAmt
